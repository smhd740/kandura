...
class StoreOrderRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // Measurements validation (at least one required)
            'items.*.measurement_ids' => 'required|array|min:1',
            'items.*.measurement_ids.*' => 'required|exists:measurements,id|distinct',

            // Design Options validation (optional, can be empty array)
            'items.*.design_option_ids' => 'nullable|array',
            'items.*.design_option_ids.*' => 'required|exists:design_options,id|distinct',
        ];
    }
    ...
    public function messages(): array
    {
        return app()->getLocale() === 'ar' ? [
            ...
            'items.*.measurement_ids.*.distinct' => 'لا يمكن تكرار نفس المقاس',
            ...
            'items.*.design_option_ids.*.distinct' => 'لا يمكن تكرار نفس خيار التصميم',
        ] : [
            // Address
            ...
            'items.*.measurement_ids.*.distinct' => 'Cannot select the same size twice',
            ...
            'items.*.design_option_ids.*.distinct' => 'Cannot select the same design option twice',
        ];
    }
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            // Validate that all designs are active
            if ($this->items) {
            ...
            // Validate that all design options are active
            if ($this->items) {
                foreach ($this->items as $index => $item) {
                    if (isset($item['design_option_ids'])) {
                        foreach ($item['design_option_ids'] as $optionIndex => $optionId) {
